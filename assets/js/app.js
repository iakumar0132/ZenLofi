...
function createPlayList() {
  songList.forEach((song, index) => {
    let button = createEle("button"); // Create button element
    let li = createEle("li");

    li.classList.add("track-item");
    button.innerText = song.name; // Set button text to song name
    button.classList.add("track-button"); // Add a class to style the button
    button.setAttribute("data-index", index); // Set data-index attribute to track index
    button.addEventListener("click", playSelectedSong); // Add click event listener
    append(li, button);
    append(ul, li);
  });
  append(musicbox, ul);
